...
public class ElJugadorCercaVProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (entity instanceof VillagerEntity) {
			if (!world.getEntitiesOfClass(Player.class, AABB.ofSize(new Vec3(x, y, z), 50, 50, 50), e -> true).isEmpty()) {
				if (Mth.nextInt(RandomSource.create(), (int) (double) ConfiguracionConfiguration.RAINVILLAGERMIN.get(), (int) (double) ConfiguracionConfiguration.RAINVILLAGERMAX.get()) == 1) {
					if (world instanceof ServerLevel _level) {
						Entity entityToSpawn = new Villager(EntityType.VILLAGER, _level);
						entityToSpawn.moveTo(x, (y + 45), z, 0, 0);

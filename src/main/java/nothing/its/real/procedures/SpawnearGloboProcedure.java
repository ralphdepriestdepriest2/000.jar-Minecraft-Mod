...
public class SpawnearGloboProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z) {
		if (Ashes1ashesModVariables.MapVariables.get(world).tiempoEstructuras == 0) {
			if (ConfiguracionConfiguration.ENABLEBALLOON.get()) {
				if (!(!world.getEntitiesOfClass(BalloonEntity.class, AABB.ofSize(new Vec3(x, y, z), 4000, 4000, 4000), e -> true).isEmpty())) {
					if (world.canSeeSkyFromBelowWater(BlockPos.containing(x, y, z))) {
						if (Mth.nextInt(RandomSource.create(), 1, 2) == 1) {
							if (world instanceof ServerLevel _level) {
								Entity entityToSpawn = new BalloonEntity(Ashes1ashesModEntities.BALLOON.get(), _level);
								entityToSpawn.moveTo((x + 20), (y + 20), z, 0, 0);
